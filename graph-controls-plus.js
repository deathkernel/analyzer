(() => {
  ...
  const svg = () => $('graphSvg');
  let zoom=1,panX=0,panY=0,panning=false,startX=0,startY=0;
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    s.addEventListener('wheel',e=>{
      ...
      zoom=Math.max(.45,Math.min(3,zoom*factor));
      applyView();
    },{passive:false});
    box.addEventListener('mousedown',e=>{
      if(e.target.closest('.node')||e.target.closest('button')||!e.shiftKey)return;
      panning=true;
    ...
    window.addEventListener('mousemove',e=>{
      if(!panning)return;
      panX=e.clientX-startX;panY=e.clientY-startY;applyView();
    });
