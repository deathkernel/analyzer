...
def test_exception_engine_does_not_abort_on_bad_python():
    with TemporaryDirectory() as tmp:
        root=Path(tmp);path=root/"broken.py"
...
def test_graph_resolves_qualified_python_import_before_duplicate_stem():
    with TemporaryDirectory() as tmp:
        root=Path(tmp)
        (root/"pkg").mkdir();(root/"other").mkdir()
        ...
        assert "@file:pkg/alpha.py" in targets
        assert "@file:other/alpha.py" not in targets
...
def test_graph_does_not_guess_ambiguous_duplicate_stems():
    with TemporaryDirectory() as tmp:
        ...
        (root/"b"/"util.py").write_text("x=2\n",encoding="utf-8")
        (root/"main.py").write_text("import util\n",encoding="utf-8")
        files=[p for p in root.rglob("*.py")]
        contents={p:p.read_text(encoding="utf-8") for p in files}
        _,edges=build_graph(root,files,contents)
...
def test_scanner_ignores_secret_fixture_in_test_context():
    with TemporaryDirectory() as tmp:
        root=Path(tmp);tests=root/"tests";tests.mkdir()
        (tests/"test_auth.py").write_text('password = "supersecret123"\n',encoding="utf-8")
        result=analyze(root)
...
def test_scanner_only_flags_high_confidence_dynamic_innerhtml():
    with TemporaryDirectory() as tmp:
        root=Path(tmp)
        (root/"safe.js").write_text('element.innerHTML = escapeHtml(value);\n',encoding="utf-8")
        (root/"unsafe.js").write_text('element.innerHTML = location.search;\n',encoding="utf-8")
        result=analyze(root)
        safe=[x for x in result["issues"] if x["file"]=="safe.js" and x["type"]=="DOM_XSS"]
        unsafe=[x for x in result["issues"] if x["file"]=="unsafe.js" and x["type"]=="DOM_XSS"]
        assert safe == []
        assert len(unsafe) == 1


def test_graph_resolves_multiple_from_import_names():
    with TemporaryDirectory() as tmp:
        root=Path(tmp);pkg=root/"pkg";pkg.mkdir()
        (pkg/"alpha.py").write_text("x=1\n",encoding="utf-8")
        (pkg/"beta.py").write_text("y=2\n",encoding="utf-8")
        main=root/"main.py";main.write_text("from pkg import alpha, beta\n",encoding="utf-8")
        files=[p for p in root.rglob("*.py")];contents={p:p.read_text(encoding="utf-8") for p in files}
        _,edges=build_graph(root,files,contents)
        targets={e["target"] for e in edges if e["source"]=="@file:main.py" and e["kind"]=="import"}
        assert "@file:pkg/alpha.py" in targets
        assert "@file:pkg/beta.py" in targets


def test_graph_resolves_from_dot_import_to_file_not_folder():
    with TemporaryDirectory() as tmp:
        root=Path(tmp);pkg=root/"pkg";pkg.mkdir()
        ...
        files=[p for p in root.rglob("*.py")];contents={p:p.read_text(encoding="utf-8") for p in files}
        _,edges=build_graph(root,files,contents)
        targets={e["target"] for e in edges if e["source"]=="@file:pkg/main.py" and e["kind"]=="import"}
        assert "@file:pkg/helper.py" in targets
        assert "@folder:pkg" not in targets


def test_import_tokens_preserve_python_source_order():
    tokens=import_tokens("import os\nfrom pkg import alpha\n",Path("main.py"))
    assert tokens==["os","pkg.alpha","pkg"]


def test_import_tokens_capture_all_python_from_import_names():
    tokens=import_tokens("from pkg import alpha, beta\n",Path("main.py"))
    assert tokens==["pkg.alpha","pkg.beta","pkg"]


def test_import_tokens_encode_relative_from_import_name():
    tokens=import_tokens("from . import helper\n",Path("pkg/main.py"))
    assert tokens==[".helper","."]
